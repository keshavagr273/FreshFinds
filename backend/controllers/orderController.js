    ...
    for (const item of cart.items) {
      ...
      if (!liveProduct) {
        ...
        for (const processedItem of processedItems) {
          await Product.findByIdAndUpdate(processedItem.product, {
            $inc: { 'stock.quantity': processedItem.quantity, sold: -processedItem.quantity }
          });
        }
        return res.status(400).json({
          success: false,
      ...
      processedItems.push({
        ...
        merchant: liveProduct.merchant,
        name: liveProduct.name,
        price: liveProduct.price,
        quantity: item.quantity,
        image: liveProduct.images[0]?.url || ''
      });
    }

    const orderItems = processedItems;

    // Calculate order summary using live prices
...
exports.getMerchantOrders = async (req, res) => {
  try {
    ...
    const [orders, total] = await Promise.all([
      Order.find({ 'items.merchant': req.user._id })
        .populate('customer', 'username email phone')
        .populate('items.product', 'name images')
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit),
      Order.countDocuments({ 'items.merchant': req.user._id })
    ]);
